...
class InlineLinkAdmin(BaseModelFormSet):
    '''
...
def _remove_blank_reverse_inlines(obj, formsets):
    ...
    def to_filter(formset):
        if not isinstance(formset, InlineLinkAdmin):
            return True
...
class LinkModelAdmin(ModelAdmin):
    ...
    def __init__(self, model, admin_site):
        ...
        if (hasattr(self, 'inline_pos') and self.inline_pos == 'top'):
            self.change_form_template = 'model_link/change_form.html'
    ...
    def get_inline_instances(self, request, obj=None):
        return self.tmp_inline_instances + super().get_inline_instances(request, obj)
